...
import com.example.jcsqlite.ui.components.CrDialogInfo
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            JCSQLiteTheme {
                ...
                var parks by remember { mutableStateOf(listOf<Park>()) }
                            ...
                            if (!db.updatePark(park)){
                                scope.launch {
                                    snackbarHostState.showSnackbar(
                                        getString(R.string.main_error_uopdate),
                            ...
                            onDismissRequest = { delete ->
                                if (delete){
                                    if (db.deletePark(park)){
                                        parks = parks.toMutableList().also { it.remove(park) }
                                    }else{
                                        scope.launch {
                                            snackbarHostState.showSnackbar(
                                                getString(R.string.main_error_uopdate),
